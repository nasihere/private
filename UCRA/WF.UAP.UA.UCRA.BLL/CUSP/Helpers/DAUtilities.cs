...
namespace WF.EAI.Web.Controls.uiUtils.uuw
{
    ...
    //using WF.EAI.Web.Controls.controls.uca;
    ...
    public class DAUtilities
    {
        ...
        public static CuspData BuildBaseCUSPAppHeader(User user, string applicationId)
        {
            CuspData header = BuildBaseCUSPAppHeader(user);
            header.ApplicationId = applicationId;
            return header;
        }
        ...
        public static CuspData BuildBaseCUSPAppHeader(User user)
        {
            CuspData header = new CuspData();
        ...
        /// <returns>
        /// </returns>
        public static AppDataHeader BuildAppHeader(User user, string applicationId)
        {
            AppDataHeader header = BuildAppHeader(user);
            header.ApplicationId = applicationId;
            return header;
        }
        ...
        public static string FieldModeStyleClass(string css, FieldAttribute.FieldModes fieldMode)
        {
            ...
            sBuilder.Append(oldCss);
            sBuilder.Append(" ");
            sBuilder.Append(fieldMode.ToString());
        ...
        public static void SetBindingForTextbox(TextBox fieldControl, FieldAttribute fieldAttribute)
        {
            if (fieldAttribute != null)
            {
                ...
                fieldControl.Attributes.Add("AcapsFieldName", fieldAttribute.AcapsFieldName);
                fieldControl.Attributes.Add("PageNumber", fieldAttribute.PageNumber);
                ...
                fieldControl.Attributes.Add("OriginalValue",fieldAttribute.OriginalValue);
                fieldControl.CssClass = FieldModeStyleClass(fieldControl.CssClass, fieldAttribute.FieldMode);
        ...
        public static void SetBindingForTextbox(TextBox fieldControl, FieldAttribute fieldAttribute, string addlOnChangeJs)
        {
            if (fieldAttribute != null)
            {
                fieldControl.CssClass = FieldModeStyleClass(fieldControl.CssClass, fieldAttribute.FieldMode);
                ...
                if (!fieldAttribute.IsProtected)
                {
                    var script = string.Concat(GetJsonFunction(fieldAttribute.AcapsFieldName,
                                                                          fieldAttribute.PageNumber,
                                                                          fieldAttribute.Value)
                                                ,addlOnChangeJs);
                    fieldControl.Attributes.Add("onchange", script);
                }
            }
            else
            {
        ...
        public static void SetBindingForDatePicker(TextBox fieldControl, FieldAttribute fieldAttribute, ImageButton imageBtn)
        {
            ...
            if (fieldAttribute.IsProtected)
            {
                imageBtn.ImageUrl = "~/Common/Images/1x1.gif";
        ...
        public static void SetBindingForSalesPlanReadOnlyTextbox(TextBox fieldControl, FieldAttribute fieldAttribute)
        {
            ...
            fieldAttribute.IsProtected = true;
            //fieldControl.ReadOnly = true;
            fieldControl.Attributes.Add("unselectable", "on");
        ...
        /// <param name="fieldControl"></param>
        /// <param name="fieldAttribute"></param>
        public static void SetBindingForSalesPlanReadOnlyTextbox(TextBox fieldControl, FieldAttribute fieldAttribute, DropDownList ddlCtl)
        {
        ...
        public static void DisableChildControls(Control tabControl)
        {
            ...
            {
                if (cntl != null)
                    DAUtilities.DisableTabControls(cntl);
            }
        }
        ...
        public static void DisableTabControls(Control cntl)
        {
            if (cntl is TextBox)
            {
                ...
                ((TextBox)cntl).CssClass = DAUtilities.FieldModeStyleClass(((TextBox)cntl).CssClass, FieldAttribute.FieldModes.Protected_Normal);
            }
            else if (cntl is DropDownList)
            {
                ((DropDownList)cntl).Enabled = false;
                ((DropDownList)cntl).CssClass = DAUtilities.FieldModeStyleClass(((DropDownList)cntl).CssClass, FieldAttribute.FieldModes.Protected_Normal);
            }
            else if (cntl is CheckBox)
            {
                ((CheckBox)cntl).Enabled = false;
                ((CheckBox)cntl).CssClass = DAUtilities.FieldModeStyleClass(((CheckBox)cntl).CssClass, FieldAttribute.FieldModes.Protected_Normal);
            }
            ...
            else if (cntl is GridView)
            {
                GridView radDebGrid = cntl as GridView;
                foreach (GridViewRow item in radDebGrid.Rows)
                {
                    foreach (Control cntItem in item.Controls)
                    {
                        foreach (Control cnt in cntItem.Controls)
                        {
                            if (cnt is TextBox)
                            {
                                ...
                                ((TextBox)cnt).CssClass = DAUtilities.FieldModeStyleClass(((TextBox)cnt).CssClass, FieldAttribute.FieldModes.Protected_Normal);
                            }
                            else if (cnt is DropDownList)
                            {
                                ((DropDownList)cnt).Enabled = false;
                                ((DropDownList)cnt).CssClass = DAUtilities.FieldModeStyleClass(((DropDownList)cnt).CssClass, FieldAttribute.FieldModes.Protected_Normal);
                            }
                            else if (cnt is CheckBox)
                            {
                                ((CheckBox)cnt).Enabled = false;
                                ((CheckBox)cnt).CssClass = DAUtilities.FieldModeStyleClass(((CheckBox)cnt).CssClass, FieldAttribute.FieldModes.Protected_Normal);
                            }
        ...
        {
            var itemsUpperCase = new ListItemCollection();
            foreach (ListItem listItem in fieldControl.Items)
            {
                itemsUpperCase.Add(new ListItem { Text = listItem.Text.ToUpper(), Value = listItem.Value.ToUpper() });
            }
            var itemIndex = itemsUpperCase.IndexOf(itemsUpperCase.FindByValue(fieldAttribute.Value.ToUpper()));

            if (itemIndex != -1)
            {
                fieldControl.SelectedIndex = -1;
                fieldControl.Items[itemIndex].Selected = true;
            }
            else
            {
                itemIndex = itemsUpperCase.IndexOf(itemsUpperCase.FindByText(fieldAttribute.Value.ToUpper()));
                if (itemIndex != -1)
                {
                    fieldControl.SelectedIndex = -1;
                    fieldControl.Items[itemIndex].Selected = true;

...
function DoReports($p){
	...
	$qry = "SELECT employee_id,DAYNAME(devicetime) as day,min(devicetime) as inTime FROM elog WHERE action = 0 AND employee_id = '".$p->report->empname."' AND session_id = '".$p->session_id ."' GROUP BY date(devicetime)";
	$data[0]=get_col_array($qry);
	//Return day and In Time
	$qry = "SELECT employee_id,max(devicetime) as outTime FROM elog WHERE action = 1 AND employee_id = '".$p->report->empname."' AND session_id = '".$p->session_id ."' GROUP BY date(devicetime)";
	$data[1]=get_col_array($qry);
	...
	foreach($data[0] as $v1)
	{
		...
		foreach($data[1] as $v2)
		{
			...
			if($date1 == $date2)
			{
				...
				$dataOfDepartSummary['log'][$j]['report']['day']=$v1['day'];
				$dataOfDepartSummary['log'][$j]['report']['date']=$date1;
				//$dataOfDepartSummary[$i]['report']['date2']=$date2;
				$dataOfDepartSummary['log'][$j]['report']['inTime']=date('h:i:s a', strtotime($v1['inTime']));
				$dataOfDepartSummary['log'][$j]['report']['outTime']=date('h:i:s a', strtotime($v2['outTime']));
				$dataOfDepartSummary['log'][$j]['report']['noOfhours']=$hours[0]['noOfhours'];
				$dataOfDepartSummary['log'][$j]['report']['pic'] = getImageURL($v2['outTime']."N-A-N".$v1['employee_id']."N-A-N".$p->session_id);
				$j++;
			}
		}
	}

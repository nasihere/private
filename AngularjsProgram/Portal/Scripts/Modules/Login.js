...
angular.module('Console.login', ['ui.router', 'ngStorage'])
...
.service('Session', function () {
    ...
    this.createByObject = function (res) {
        ...
        ConsoleConfig.tokenId = res.token;
    };
    this.setSessionTimeOut = function(timeoutminute) {
        ...
        .factory('AuthService', function ($http, Session, $localStorage) {
            ...
            authService.GetAccountModules = function (userinfo,tokenid) {
                var url = ConsoleConfig.baseurl + ConsoleConfig.AccountInfo;
                return $http({
                    method: 'POST',
                    url: url,
                    ...
                    SmallBox("Console - " + er.error, "<b>" + er.error_description + "</b>", "error");
            ...
            authService.login = function (credentials) {
                var url = ConsoleConfig.baseurl + ConsoleConfig.TOKEN;
                return $http({
                    method: 'POST',
                    url: url,
    ...
    return {
        ...
        validatelogin: function () {
            ...
            if (username === '') {
                ...
                response.error = true;
                response.errmsg = "Please enter your temp";
            }
            else {
                response.error = false;
            }
...
.controller('LoginController', function ($scope, $rootScope, AUTH_EVENTS, AuthService, $state, LoginFactory, $localStorage, Session) {
    ...
    $scope.login = function (credentials) {
        ...
        AuthService.login(credentials).then(function (res) {
           ...
           AuthService.GetAccountModules(credentials, tokenId).then(function (res) {
                ...
                var user = $scope.fillUpConsoleModel(res.data.AccountModel, tokenId);
    ...
    $scope.fillUpConsoleModel = function (acctDetails, token) {
        var user = ConsoleModal;
    ...
    $scope.AdminMenu = function (user) {
        if (user.role == "admin") {
             user.linkAccess.push(ConsoleAdminMenu);
        }
        return user;
    };

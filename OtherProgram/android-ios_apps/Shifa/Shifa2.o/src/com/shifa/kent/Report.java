...
public class Report {
    ...
    public void Init() {
        cursor = db1.getReadableDatabase().rawQuery("Select _id,Remedies,Name,categoy,maincategoy from .XXXXXXX where selected = '1'", null);
        ...
        while (!cursor.isAfterLast()) {
            ...
            try {
                for (int i = 0; i <= sRemediesSplit.length - 1; i++) {
                    ...
                    if (sTemp == null) break;
                    String q = "Insert Into .XXXXXXXreport (sync, remedies,  intensity, _idd,Name,maincategoy) values (0,'" + sTemp[0] + "'," + sTemp[1] + "," + _id + ",'" +
                            _name + "','" + _maincategory + "')";

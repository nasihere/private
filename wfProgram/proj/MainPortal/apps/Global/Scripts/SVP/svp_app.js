...
var clearJForm = function (obj) { // ClearJsonForm is used to clear the clear all the values
    
    $.each(obj, function (i, item) {
        var ords = typeof item;
        if (ords == "object") {
...
var svpApp = {
     ...
     //   svpApp.util.number10mask();
      ...
      /*  $('.select').on("focus", "select", function () {
        ...
        */
        ...
        if ($('div.text-truncate').length > 0) { svpApp.util.textTruncateListener(); }
        ...
        if ($('table.table-selectable').length > 0) { svpApp.util.tableSelectable(); }

        if ($('.searchTable').length > 0) { svpApp.util.searchTable(); }
        ...
        // home/dashboard only
        if ($('body#dashboard').length > 0) { svpApp.dashboard.init(); }

        // customer_record only
        if ($('body#customer_record').length > 0) { svpApp.customerRecord.init(); }

        // required_customer only
        if ($('body#required_customer').length > 0) { svpApp.requiredCustomer.init(); }

        // product_selection only
        if ($('body#product_selection').length > 0) { svpApp.productSelection.init(); }

        // product_acceptance only
        if ($('body#product_acceptance').length > 0) { svpApp.productAcceptance.init(); }

        // product_fulfillment only
        if ($('body#product_fulfillment').length > 0) { svpApp.productFulfillment.init(); }

        // signature capture only
        if ($('#signature-fillable').length > 0) { svpApp.util.captureSignature(); }

        // account details only
        if ($('body#account_detail').length > 0) { svpApp.accountDetail.init(); }

        // search results only
        if ($('body#search_results').length > 0) { svpApp.searchResults.init(); }
        ...
        if ($('body#prod_confirm').length > 0) { svpApp.prodConfirm.init(); }
    ...
    modal:
        ...
        init: function () {
            ...
            $('a.editlink').unbind('click').on('click', function (e) {
                OpenModal(this);
            });
        ...
        callback: function () {
            ...
            if ($('#acctHistory').length > 0) { setupDataTable('acctHistory'); }
            if ($('#autoFinanceLoanHistory').length > 0) { setupDataTableLoans('autoFinanceLoanHistory'); }
            if ($('#homeEquityLoanHistory').length > 0) { setupDataTableLoans('homeEquityLoanHistory'); }
            if ($('#installmentHistory').length > 0) { setupDataTableLoans('installmentHistory'); }
            if ($('#lineOfCreditHistory').length > 0) { setupDataTableLoans('lineOfCreditHistory'); }
            if ($('#homeEquityLineHistory').length > 0) { setupDataTableLoans('homeEquityLineHistory'); }
            if ($('#acctDetATMHistory').length > 0) { setupDataTableATM('acctDetATMHistory'); }
            if ($('#atworkresults_modal').length > 0) { svpApp.util.tableSelectable_atWorkCode(); }
            ...
            */
            ...
            if ($('#customer_recommendations').length > 0) { svpApp.customerRecommendations.init(); };
            if ($('#customer_snapshot').length > 0) { svpApp.customerSnapshot.init(); };
            //spf - 04/22/2014 - need to initalize dashboard functions here after the modal_dashboard opens.
            if ($('#dashboard_modal').length > 0) { svpApp.dashboard.init(); };
            if ($('#customer_record').length > 0) { svpApp.util.datePicker(); };
    ...
    util: {
           ...
           */
            $('select.select').each(function () {
               var title = $(this).attr('title');
                ...
                $(this)
                    .css({ 'z-index': 10, 'opacity': 0, '-webkit-appearance': 'none',  'min-width': "82px" })
                    .blur(function () {
                        var widthItem = $(this).next().width();
                        widthItem = widthItem + 60;
                        if (widthItem < 30) {
                            widthItem = "112";
                        }
                ...
                if ($(this).hasClass('disabled') || ($(this).attr('disabled') == 'disabled')) {
                    ...
                    $(this).after('<span  class="select titleselect">' + title + '</span>');
                }
               
            });
            setTimeout(function () {
                svpApp.util.buildWidth();
            }, 200);

        },
        buildWidth: function () {
            $('.titleselect').each(function () {
                ...
                $(this).prev().css("width", widthItem + "px");
        ...
        employmentInfo: function () {
            $('.js-teammember').on('change', function () {
                ...
                switch (pickedval) {
                    case "yes":
                        $('div.tab-pane.active .js-employertxt').val('xxxxProjectxxx');
        ...
        init: function () {
            ...
            if (privateTab) {
                if ($('li.privbnkcust').hasClass('active')) {
                    $('.tab-content').addClass('private-bank'); $('li.privbnkcust').removeClass('privbnk');
                } else {
                    $('.tab-content').removeClass('private-bank'); $('li.privbnkcust').addClass('privbnk');
                };
            };

...
angular.module('App.Validation', [])
    .directive('valid', ['$parse', function ($parse) {
    return {
        restrict: 'A',
...
.directive('reset', [function ($parse) {
    return {
        restrict: 'A',
        require:'ngModel',
...
/*
    ...
    angular.forEach(pageError.errorSummary, function (object, index) {
        if (object != undefined && object.errorMsg != "") {
            ...
            $("[vid='" + object.errorId + "']").closest("label").css("border", "1px solid red");
            $("[vid='" + object.errorId + "']").closest(".panel-default").css("border", "1px solid red");
            setTimeout(function() {
                $("[vid='" + object.errorId + "']").addClass('ng-invalid');
                $("[vid='" + object.errorId + "']").addClass('ng-valid');
                
                $("[vid='" + object.errorId + "']").closest("label").css("border", "");
